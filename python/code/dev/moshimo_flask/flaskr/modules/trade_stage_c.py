...
print(f'start::::trade_stage_c date is {date}')
        ...
        if order_price <= high_price and order_price >= high_price:
            match app_rtype:
                case 'simu':
                    ...
                    order_id,tmp_r_funds, tmp_diff_funds)
                    ...
                    TBL_VAL.tbl_upd_order_after_exe(order_id, 'off')
                case 'real':
                    print('あとで実装')
...
if not order_id is None: #o idはnoneじゃない？
    ...
    if tmp_edit_price == 1:
        match app_rtype:
            case 'simu':
                #update order
                TBL_VAL.tbl_upd_order_price(order_id,tmp_order_price)
                #update fund
                #fundで最新より1つ前のrを取得
                fund_before_r_funds = TBL_VAL.tbl_before_fund_r_funds(trade_id)
                #あとは新規order時のように算出してupdate
    ...
    match app_rtype:
            case 'simu':
                ...
                TBL_VAL.tbl_ins_order(trade_id, 1, 'on', 'buy', 
                        tmp_order_price, tmp_order_quantity, tmp_pf_order_number)
                ...
                #def:::status, residual_funds, update_diff_funds
                TBL_VAL.tbl_ins_fund(trade_id, order_id, 0, app_rtype, ticker, 
                        'on', tmp_r_funds, tmp_diff_funds)
            case 'real':
                print('あとで実装')
